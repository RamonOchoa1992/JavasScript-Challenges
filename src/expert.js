...
export const isAuthenticSkewer = str => {
    ...
    strArr.forEach((el, ind, arr) => {
        if (/[A-Z]/.test(el)) {
            letters.push(el);
            counterL++;
            if (ind === arr.length - 1 || !/[A-Z]/.test(arr[ind + 1])) {
                letterCounter.push(counterL);
                counterL = 0;
            }
        }
        else if (/-/.test(el)) {
            counter++;
            if (ind === arr.length - 1 || !/-/.test(arr[ind + 1])) {
                skewerCounter.push(counter);
                counter = 0;
            }
        }
        else {
            incorrectChar.push(el);
        }
    })

    const greaterThanOne = letterCounter.filter(el => el > 1);

    if (letters.length === 0 || skewerCounter.length === 0) return { state: false, response: "You can't enter strings without any actual skewer '-' or letters" };
    if (incorrectChar.length > 0) return { state: false, response: "You can't enter no valid characters. Capital letters and '-' char only." }
    if (greaterThanOne.length > 0) return { state: false, response: "You can't enter two letter in a row, they must be separated by '-' char." }

    const initialChar = strArr[0];
    const finalChar = strArr[strArr.length - 1];

    const isVowel = letter => ["A", "E", "I", "O", "U"].includes(letter);
    const isConsonant = letter => !isVowel(letter) && !/-/.test(letter);

    if (!isConsonant(initialChar) || !isConsonant(finalChar)) return { state: false, response: "Should start and end with a consonant." }

    const consonants = letters.map((el, index) => [isConsonant(el), index])
    const indexConsonants = consonants.filter(el => el[0]).map(el => el[1])

    const vowels = letters.map((el, index) => [isVowel(el), index])
    const indexVowels = vowels.filter(el => el[0]).map(el => el[1])

    if (indexConsonants.filter(el => el % 2 !== 0).length > 0 || indexVowels.filter(el => el % 2 === 0).length > 0) return { state: false, response: "Should alternate between consonants and vowels." }

    let counterS = 0;
    for (let i = 0; i < skewerCounter.length - 1; i++) {
        if (skewerCounter[i] === skewerCounter[i + 1]) {
            counterS++;
        }
    }

    if (counterS !== skewerCounter.length - 1) return { state: false, response: "Should have consistent spacing between letters." }

    return { state: true, response: "All okay" };
}

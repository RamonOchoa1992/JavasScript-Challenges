...
export const cFuge = (hole, tube) => {
    if (hole === 1) return {
        state: false, hole: hole,
        tube: tube, response: "One hole is never in balance, even empty."
    };
    if (tube === 1) return {
        state: false, hole: hole,
        tube: tube, response: "One test tube is never in balance."
    }
    ...
    const response = haveCommonFactor(hole, tube)
        ? `The tube ${tube} is included in: '${[...multArray].sort((a, b) => a - b).join(", ")}'`
        : `The tube ${tube} is not included in: '${[...multArray].sort((a, b) => a - b).join(", ")}'`
